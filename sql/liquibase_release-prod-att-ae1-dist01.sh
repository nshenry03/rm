...
then
        ...
        echo "Example:  ./liquibase_export.sh bulk 190 # This is for bulk DB"
        exit
fi
...
appdirect)  echo "Getting SQL for AppDirect DB for Release $VERSION"
    ...
    CONF=appdirect.conf
    CMD="cd /home/aduser/liquibase/xml/appdirect/ ; /usr/bin/liquibase --logLevel=severe --defaultsFile=$CONF_DIR/$CONF --changeLogFile=$TARGET_DIR/db.changelog-master.xml updateSQL > $SQL_DIR/$REPORT"
    ;;
jbilling)  echo  "Getting SQL for JBilling DB for Release $VERSION"
    TARGET_DIR="/home/aduser/liquibase/xml/jbilling/"
    CONF=jbilling.conf
    CMD="cd /home/aduser/liquibase/xml/jbilling/ ; /usr/bin/liquibase --logLevel=severe --defaultsFile=$CONF_DIR/$CONF --changeLogFile=$TARGET_DIR/jbilling-changeLog.xml updateSQL > $SQL_DIR/$REPORT"
    ;;
bulk)  echo  "Getting SQL for JBilling DB for Release $VERSION"
    TARGET_DIR="/home/aduser/liquibase/xml/bulk/liquibase"
    CONF=bulk.conf
    CMD="cd /home/aduser/liquibase/xml/bulk/ ; /usr/bin/liquibase --logLevel=severe --defaultsFile=$CONF_DIR/$CONF --changeLogFile=$TARGET_DIR/db.changelog-master.xml updateSQL > $SQL_DIR/$REPORT"
    ;;
esac
