...
appdirect)  echo "Getting SQL for AppDirect DB for Release $VERSION"
    ...
    CONF=appdirect.conf
    CMD="cd /home/aduser/liquibase/xml/appdirect/ ; /usr/bin/liquibase --defaultsFile=$CONF_DIR/$CONF --changeLogFile=$TARGET_DIR/db.changelog-master.xml updateSQL > $SQL_DIR/$REPORT"
    ;;
jbilling)  echo  "Getting SQL for JBilling DB for Release $VERSION"
    TARGET_DIR="/home/aduser/liquibase/xml/jbilling/"
    CONF=jbilling.conf
    CMD="cd /home/aduser/liquibase/xml/jbilling/ ; /usr/bin/liquibase --defaultsFile=$CONF_DIR/$CONF --changeLogFile=$TARGET_DIR/jbilling-changeLog.xml updateSQL > $SQL_DIR/$REPORT"
    ;;
esac
